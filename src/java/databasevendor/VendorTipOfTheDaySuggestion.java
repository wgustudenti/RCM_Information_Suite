...
import sqlitedata.SQLiteTipOfTheDaySuggestion;
 ...
 * the vendor.
 */
public abstract class VendorTipOfTheDaySuggestion {
    ...
    public static void insert(String description) throws SQLException {
        if(SQLConnection.getVendor().toLowerCase().contains("sqlite")){
            SQLiteTipOfTheDaySuggestion.insert(description);
        }
    }
    ...
    public static void update(String description, int primaryKey) throws SQLException {
        if(SQLConnection.getVendor().toLowerCase().contains("sqlite")){
            SQLiteTipOfTheDaySuggestion.update(description, primaryKey);
        }
    }
    ...
    public static ResultSet getAll() throws SQLException {
        if(SQLConnection.getVendor().toLowerCase().contains("sqlite")){
            return SQLiteTipOfTheDaySuggestion.getAll();
        }
        return null;
    }
    ...
    public static void delete(int primaryKey) throws SQLException {
        if(SQLConnection.getVendor().toLowerCase().contains("sqlite")){
            SQLiteTipOfTheDaySuggestion.delete(primaryKey);
        }
    }
