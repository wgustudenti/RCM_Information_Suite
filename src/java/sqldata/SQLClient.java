...
public abstract class SQLClient {
    ...
    public static void insert(String name) throws SQLException {
        name = SQLGeneral.escapeApostrophes(name);
     ...
     * @throws SQLException
     */
    public static void update(String name, String departmentNotes, String specialtyNotes, int primaryKey) throws SQLException {
        name = SQLGeneral.escapeApostrophes(name);
        if(departmentNotes != null) {
            departmentNotes = SQLGeneral.escapeApostrophes(departmentNotes);
        }
        else{
            departmentNotes = "";
        }
        if(specialtyNotes != null) {
            specialtyNotes = SQLGeneral.escapeApostrophes(specialtyNotes);
        }
        else{
            specialtyNotes = "";
        }
        String query = "UPDATE " + tableName + " SET Name='" + name + "', DepartmentNotes='"
                + departmentNotes + "', SpecialtyNotes='" + specialtyNotes
    ...
    public static void createTable() throws SQLException {
        String query = "CREATE TABLE " + tableName + " (\n" +
                ...
                "\tDepartmentNotes VARCHAR[10000],\n" +
                "\tSpecialtyNotes VARCHAR[10000]\n" +
                ");";
        SQLGeneral.executeUpdate(query);
    }
    ...
    public static void insertTestData() throws SQLException {
        insert("The Good Client");
        insert("The Bad Client");
        insert("The Ugly Client");
    }
}
